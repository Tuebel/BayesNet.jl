...
TODO
"""
struct ObservationNode{name,child_names,N<:BroadcastedNode{name,child_names},O} <: AbstractNode{name,child_names}
    wrapped_node::N
    observation::O
end
...
function logdensityof_barrier(node::ObservationNode, variables::NamedTuple)
    ...
    ℓ = logdensityof(node.wrapped_node(child_values...), node.observation)
